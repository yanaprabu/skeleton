...
class Pagination_Adapter_DbTest extends UnitTestCase	{
	...
	public function setUp()	{
		...
		$this->db->expect('limit', array('*', 10, 9));		// 9 because LIMIT offset is zero based
	...
	public function testGetItemsDelegatesToDatasource()	{
		$this->db->expectOnce('limit', array($this->query, 10, 9));		// 9 because LIMIT offset is zero based
	...
	public function testSetOrderByFieldAscending()	{
		$this->adapter->setOrderBy('title');
		$this->db->expectOnce('limit', array($this->query . ' ORDER BY title ASC', 10, 9));		// 9 because LIMIT offset is zero based
		$this->adapter->getItems(10, 10);
	}

	public function testSetOrderByFieldDescending()	{
		$this->adapter->setOrderBy('title', 1);
		$this->db->expectOnce('limit', array($this->query . ' ORDER BY title DESC', 10, 9));		// 9 because LIMIT offset is zero based
		$this->adapter->getItems(10, 10);
	}
